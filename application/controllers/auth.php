...
class Auth extends MY_Controller {
	...
	function login()
	{
		...
		if ($this->form_validation->run() == true)
		{
			...
			//remember me is disabled because it didn't remember from day to day.
			//therefore remember is always false
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
			{
				...
				//$this->data['content'] = $this->load->view('members/member_home_view', $this->data, TRUE);;

				//$this->load->view('members/member_view', $this->data);
			}
			else
			{
				//if the login was un-successful
				//redirect them back to the login page
				$identity = $this->user_model->fetch_identity($this->input->post('identity'));
				...
				if ($identity->active == 1)//awaiting approval
				{
					$data['content'] = $this->load->view('public/public_not_approved_view', $this->data, TRUE);

					$this->load->view('public/public_view', $data);
				}
				else
				{
					if ($identity->active > 1)//approved but new password not set
					{
						if ($identity->password == '')
						{
							$this->session->set_flashdata('email', $_POST['identity']);
							redirect('general/initial_setup', 'refresh');
							exit;
						}
						else //login failed
						{
		...
		else
		{
			...
			$this->data['content'] = $this->load->view('public/public_login_view', $this->data, TRUE);
