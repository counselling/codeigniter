...
class Ion_auth_model extends CI_Model
{
	...
	public function hash_password_db($id, $password, $use_sha1_override=FALSE)
	{
        ...
        $row = $query->row();
		//$query = $this->db->prepare("SELECT password, salt FROM users WHERE id = $id");
		//$query->execute();
		...
		*/
		...
		//$row = $query->fetch(PDO::FETCH_ASSOC);
		$hash_password_db = $row->password.$row->salt;
		...
		else
		{
			...
			//$db_password = $hash;
			//$db_password = $this->passwordhash->CheckPassword($_POST['password'], $hash_password_db);
			
		}

		//if($db_password == $hash_password_db->password)
		if($this->passwordhash->CheckPassword($_POST['password'], $hash_password_db))
		{
			return TRUE;
		}
		else
	...
	public function forgotten_password($identity)
	{
		...
		//);
		//echo $identity. ' '.$key. ' '.time();
		//$forgotten_password_code = $key;
		//$forgotten_password_time = time();
		//$email = $identity;
		
		$sql = "UPDATE users
							SET	forgotten_password_code = :forgotten_password_code,
									forgotten_password_time = :forgotten_password_time
							WHERE email = :email";
		
		$stmt = $this->db->prepare($sql);

		//$stmt = $this->db->prepare("UPDATE users SET forgotten_password_code = ?, forgotten password_time  = ? WHERE email = ? LIMIT 1");
		$stmt->bindParam(':forgotten_password_code', $key, PDO::PARAM_STR);
		$stmt->bindParam(':forgotten_password_time', time(), PDO::PARAM_STR);
		$stmt->bindParam(':email', $identity, PDO::PARAM_STR);
		$stmt->execute();
...
//echo $return;
	...
	public function login($identity, $password, $remember=FALSE)
	{
        ...
        $this->db->select('first_name, last_name, email, id, password, active, last_login');
		$email = $_POST['identity'];
        $this->db->where('email', $email);
        $stmt = $this->db->get('users');
        
        //$stmt = $this->db->prepare("SELECT first_name, last_name, email, id, password, active, last_login FROM users WHERE email = :email LIMIT 1");
		//$email = $_POST['identity'];
	...
	public function get_users_groups($id=FALSE)
	{
		...
		//$stmt = $this->db->prepare("SELECT group_id FROM user_groups WHERE user_id = $id LIMIT 1");
		//$stmt->execute();
	...
	**/
	...
	public function update_last_login($id)
	{
        ...
        //$query = $this->db->prepare("UPDATE `users` SET `last_login` = ? 
		//		WHERE `id` = ?");
	...
	public function set_session($user)
	{
		...
		$session_data = array(
		    ...
		    //'old_last_login'       => $user->last_login
				'group'								=> $this->ion_auth->get_users_groups($user->id),
				'identity'             => 'email',
